...
function Index() {
  ...
  if (isLoading) {
    return <div className="text-center p-10 font-bold text-muted-foreground animate-pulse">{t('index.loading')}</div>
  }
      ...
      return (
        <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
          <div className="mb-6 text-center">
            <h2 className="text-xl font-bold text-green-600 uppercase tracking-widest text-sm">{t('index.found')}</h2>
          </div>
          <AnimalCard
            animal={foundAnimal}
            onFound={handleFound}
  ...
  if (!currentTarget) {
    return (
      <div className="text-center py-20 px-4 space-y-6 flex flex-col items-center">
        ...
        <h2 className="text-3xl font-black text-primary">{t('index.questComplete')}</h2>
        <p className="text-lg text-muted-foreground">{t('index.amazingJob')}</p>
        <Link to="/progress">
          <Button size="lg" className="mt-8 rounded-full font-bold text-lg px-8">
            {t('index.viewProgress')}
          </Button>
        </Link>
      </div>
    )
  ...
  return (
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="mb-6 text-center">
        <h2 className="text-xl font-bold text-muted-foreground uppercase tracking-widest text-sm">{t('index.currentTarget')}</h2>
      </div>
      <AnimalCard
